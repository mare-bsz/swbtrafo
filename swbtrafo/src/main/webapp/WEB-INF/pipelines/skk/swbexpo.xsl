...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="PERSON" >
		...
		<kuenstler>
			...
			<xsl:attribute name="order">
				<xsl:value-of select="java:bsz.swbexpo.trafo.SwbtrafoUtil.extendUml($label)" />
			</xsl:attribute>
	...
	<xsl:template name="expandUml">
		<xsl:param name="content"/>
		<xsl:value-of select="java:bsz.swbexpo.trafo.SwbtrafoUtil.extendUml($content)" />		
	</xsl:template>
	
	<xsl:template name="extractYear">
		<xsl:param name="datum"/>
		<xsl:value-of select="java:bsz.swbexpo.trafo.SwbtrafoUtil.extractYear($datum)" />
	</xsl:template>
